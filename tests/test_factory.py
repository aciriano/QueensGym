...
class TestSafeBoardFactory:
    ...
    def test_init(self) -> None:
        ...
        assert fact.n == 6
        assert issubclass(SafeBoardFactory, BoardFactory)
...
class TestRandomSafeBoardFactory:
    ...
    def test_init(self) -> None:
        ...
        assert fact.iters == 6
        assert fact.n == 6
        assert issubclass(RandomSafeBoardFactory, RandomBoardFactory)

    def test_init_invalid_iters(self) -> None:
        with pytest.raises(ValueError):
            RandomSafeBoardFactory(n=6, pieces=[Queen.get_id()], preplaced=(1, 1), iters=0)
    ...
    def test_new_with_final_error(self) -> None:
        fact = RandomSafeBoardFactory(
            n=100, pieces=[Queen.get_id(), Knight.get_id()], preplaced=(100, 100), iters=2
        )
    ...
    def test_new_without_final_error(self) -> None:
        fact = RandomSafeBoardFactory(n=20, pieces=[Queen.get_id()], preplaced=(17, 20), iters=1)
        results = {}
        for _ in range(100):
            try:
                board = fact.new()
